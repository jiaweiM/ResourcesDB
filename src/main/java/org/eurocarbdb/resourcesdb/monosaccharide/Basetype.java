...
public class Basetype extends MolecularEntity {
    ...
    public Basetype(String glycoCTName, Config conf, TemplateContainer container) throws ResourcesDbException {
        ...
        this.setTemplateContainer(container);
        this.init();
        GlycoCTImporter importer = new GlycoCTImporter(GlycanNamescheme.GLYCOCT, this.getConfig(), this
    ...
    /**
     * Set the ring start (carbonyl position) of the basetype. In case the ring oxygen position is defined, it is
     * adjusted to the new carbonyl position, i.e. the ring type is conserved
     *
    ...
    /**
     * This method is mainly needed for Hibernate mapping of the anomer to the database table, in which not the anomer
     * itself but its symbol is stored. If anomerStr is not null, the setAnomer(String) method is called.
     *
     ...
     * @throws MonosaccharideException in case the ring oxygen is > 0 (i.e. monosaccharide is ring form, which excludes
     *                                 alditol)
     */
    ...
    /**
     * Get the configuration that to which the anomeric center has to be compared to decide whether a monosaccharide is
     * alpha or beta
     *
    ...
    public Anomer getAnomerFromStereocode() throws ResourcesDbException {
        ...
        if (this.getRingStart() > 0 && this.isStereolossPositionWithIgnoreType(this.getRingStart(),
        ...
        for (int i = 0; i < positionsList.size(); i++) {
            ...
            if (position > 0) {
                if (position == getRingStart()) {
                    ...
                    //*** "acid" is allowed for open chain residues, other modifications are not allowed at carbonyl
                    // position ***
                    //TODO: check rules for modifications (e.g. EN is allowed)
                    if (!this.getRingtype().equals(Ringtype.OPEN) && !(mod.getTemplate().equals
                            (CoreModificationTemplate.ACID) && this.getRingStart() == 1) && !(mod.getTemplate()
                            .equals(CoreModificationTemplate.KETO)) && !(mod.getTemplate().equals
                            (CoreModificationTemplate.ENX)) && !(mod.getTemplate().equals(CoreModificationTemplate
                            .EN)) && !(mod.getTemplate().equals(CoreModificationTemplate.DEOXY))) {
                        throw new MonosaccharideException("Cannot modify monosaccharide at carbonyl position (" +
                                getRingStart() + ").");
                    }
                }
                ArrayList<CoreModification> posMods = getCoreModificationsByPosition(position); //*** list of
                // modifications, that are already present at the position ***
        ...
        /*if(pos > 0 && pos == this.getCarbonylPosition()) {
            if(mod.getTemplate().equals(CoreModificationTemplate.Acid)) {
                if(this.getRingOxygen() > 0) {
                    throw new MonosaccharideException("Cannot add 'Acid' modification to carbonyl position in ring
                    monosaccharide.");
                }
    ...
    public void deleteCoreModification(String name, int position) throws MonosaccharideException {
        ...
        throw new MonosaccharideException("Cannot remove CoreModification " + position + name + ": modification not " +
                "found");
    }
    ...
    public boolean isUronic() {
        return (!this.hasCoreModification(CoreModificationTemplate.ACID, 1) && this.hasCoreModification
                (CoreModificationTemplate.ACID, getSize()));
    }
    ...
    public boolean hasDoubleBond(int pos) {
        return (this.hasCoreModification(CoreModificationTemplate.EN, pos) || this.hasCoreModification
                (CoreModificationTemplate.ENX, pos));
    }
     ...
     * @return true, if the position has a loss of stereochemistry (not caused by the ignored modification type),
     * otherwise false
     */
    ...
    /**
     * Get a core modification specified by name and position Note: If more than one modification matches the given
     * criteria (e.g. due to uncertain positions) the first match is returned.
     *
     * @param name     the name of the core modification
        ...
        for (int pos = 1; pos <= this.getSize(); pos++) {
            ...
            for (CoreModification mod : this.getCoreModificationsByPosition(pos)) {
                if (mod.getIntValuePosition1() == 0) {
                    throw new MonosaccharideException("Cannot build atoms for basetype with uncertain core " +
    ...
    public Atom getLinkingAtom(int position, LinkageType linktype) throws ResourcesDbException {
        ...
        if (linkAtom == null) {
            throw new MonosaccharideException("Cannot get linking atom for " + linktype.name() + " linkage at " +
                    "position " + position);
        }
        if (removeAtom == null) {
            throw new MonosaccharideException("Cannot get atom to be removed for " + linktype.name() + " linkage at " +
                    "position " + position);
        }
        return linkAtom;
    }
    ...
    public void buildComposition(Config conf) throws ResourcesDbException {
        ...
        for (CoreModification mod : this.getCoreModifications()) {
            ...
            if (mod.getTemplate().equals(CoreModificationTemplate.ACID)) {
                if (mod.getIntValuePosition1() == 0) {
                    throw new MonosaccharideException("Cannot build composition for basetype with unknown acid " +
                            "position");
                } else if (this.getRingStart() == 0 && mod.getIntValuePosition1() == 1) {
                    throw new MonosaccharideException("Cannot build composition for basetype with unknown carbonyl " +
                            "position and acid modification at position 1");
                }
            ...
            //*** check for keto group at position > 1: if no keto group at position 1 and not alditol, add alditol
            // composition change (C1 of a ketose has the same composition as an alditol C1) ***
            if (mod.getTemplate().equals(CoreModificationTemplate.KETO) && mod.getIntValuePosition1() > 1) {
                if (!this.hasCoreModification(CoreModificationTemplate.KETO, 1) && !this.hasCoreModification
                        (CoreModificationTemplate.ALDITOL, 1)) {
    ...
    public void buildName() throws ResourcesDbException {
        GlycoCTExporter exporter = new GlycoCTExporter(GlycanNamescheme.MONOSACCHARIDEDB, this.getConfig(), this
                .getTemplateContainer());
        Monosaccharide ms = new Monosaccharide(this);
     ...
     * @param bt           the basetype, the properties of which are to be set. Size and ring properties of bt have to
     *                     be set before calling this method!
     * @throws ResourcesDbException in case an the extStereoStr parameter cannot be parsed or does not match the size or
     *                              ring properties of the basetye
     */
    public static void buildBasetypeByExtendedStereocode(String extStereoStr, Basetype bt) throws ResourcesDbException {
        ...
        if (extStereoStr.length() != bt.getSize()) {
            throw new MonosaccharideException("StereoString length (" + extStereoStr.length() + ") doesn't match " +
                    "basetype size (" + bt.size + ")");
        }
        ...
        for (int pos = 1; pos <= bt.getSize(); pos++) {
            ...
            if (pos == 1 && bt.getRingStart() != 1) {
                if (!buildergroup.isHeadTail()) {
                    throw new MonosaccharideException("Stereocode symbol '" + posSymbol + "' is not allowed at " +
                            "position " + pos + " (only valid for non-terminal exocyclic positions)");
                }
                ...
                throw new MonosaccharideException("Stereocode symbol '" + posSymbol + "' is not allowed at position "
                        + pos + " (only valid for terminal positions)");
            }
        ...
        for (int pos = 1; pos <= bt.getSize(); pos++) {
            ...
            if (pos == bt.getRingStart()) {
                if (buildergroup.equals(BasetypeBuilderGroup.KETO)) {
                    throw new MonosaccharideException("KETO modification not allowed at ring start position. Select D" +
                            " or L configuration to indicate orientation of OH group at anomeric center.");
                }
                if (buildergroup.equals(BasetypeBuilderGroup.CHO)) {
                    throw new MonosaccharideException("Carbonyl group not allowed at ring start position. Select D or" +
                            " L configuration to indicate orientation of OH group at anomeric center.");
                }
            }
            ...
            for (CoreModificationTemplate modTmpl : buildergroup.getCoreMods()) {
                if (modTmpl.equals(CoreModificationTemplate.EN)) {
                    ...
                    if (existEn != null && existEn.getIntValuePosition1() == pos - 1) {
                        continue; //*** current position is second position of an en modification, mod already added
                    ...
                    if (pos < bt.getSize()) {
                        ...
                        if (nextPosGroup.getCoreMods() != null && nextPosGroup.getCoreMods().contains
                    ...
                    throw new MonosaccharideException("Position " + pos + " is single EN modification position (must " +
                            "be followed or preceeded by another EN modification position)");
                    ...
                    if (existYn != null && existYn.getIntValuePosition1() == pos - 1) {
                        continue; //*** current position is second position of an yn modification, mod already added
                        // at previous position ***
                    }
                    if (pos < bt.getSize()) {
                        char nextPosSymbol = extStereoStr.charAt(pos);
                    ...
                    if (modTmpl.equals(CoreModificationTemplate.KETO) && pos > bt.getRingStart() && bt.getRingStart()
                            > 0) {
     ...
     * @throws ResourcesDbException
     */
    public void buildByExtendedStereocode(String extStereoStr, int size, int ringStart, int ringEnd) throws
