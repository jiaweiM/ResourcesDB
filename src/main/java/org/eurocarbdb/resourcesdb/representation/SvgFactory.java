...
/**
 * This class provides some general methods needed for generating SVG representations of monosaccharides and converting
 * them to other graphics formats. To actually generate the monosaccharide graphics, use the Classes that extend this
 * one (Haworth.java, Fischer.java)
 *
...
public class SvgFactory {
    ...
    private void setupFactory() {
        ...
        this.setSvgDoc((SVGDocument) domImp.createDocument("http://www.w3.org/2000/svg", "svg", null));
    ...
    /**
     * Get the top level group of the SVGGraph2D field of this object. Using this method instead of
     * getSVGGraph2D().getTopLevelGroup() preserves the group element in the SVGGraph2D object, which is replaced by a
     * new group element when the top level group is accessed directly.
     *
    ...
    protected void drawString(String str, int x, int y) {
        ...
        this.checkMinMax(x + this.getSVGGraph2D().getFontMetrics().stringWidth(str), y - this.getSVGGraph2D()
    ...
    /**
     * Get the offset that is to be added to the x position of a text to have the center of the first or last character
     * of that text at the current x position.
     *
    ...
    /**
     * Check, if the given x and y coordinates are within the current range of coordinates (min/max x/y values used by
     * the svg elements), and adjust the current range if they are not.
     *
     * @param x
     * @param y
     */
    ...
    public boolean checkSize() {
        ...
        if (this.isOutOfXBounds()) {
            if (this.isRescaleIfOutOfBounds() || this.isResizeIfOutOfBounds() || ((this.xMax - this.xMin) < this
                    .svgWidth)) {
                translateX = 0 - this.xMin;
            }
        }
        if (this.isOutOfYBounds()) {
            if (this.isRescaleIfOutOfBounds() || this.isResizeIfOutOfBounds() || ((this.yMax - this.yMin) < this
                    .svgHeight)) {
